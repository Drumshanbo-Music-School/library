...
/**
 * append-to-catalog.js
 * Appends enriched CD entries to web-app/public/data/catalog.json
 *
 * Usage:
 *   node append-to-catalog.js <input-json-file>
 *   node append-to-catalog.js --stdin < entries.json
 */
...
// Paths
const CATALOG_PATH = path.join(__dirname, 'web-app/public/data/catalog.json');
...
/**
 * Validate a CD entry has required fields
 */
function validateEntry(entry, index) {
  const required = ['id', 'type', 'title', 'artists'];
  ...
  if (!Array.isArray(entry.artists)) {
    console.error(`Entry ${index + 1}: 'artists' must be an array`);
    return false;
  }
...
function appendEntries(entries) {
  ...
  validEntries.forEach(entry => {
    console.log(`  ${entry.id}: ${entry.title} - ${entry.artists.join(', ')}`);
  });
  console.log();
}
...
Usage:
  node append-to-catalog.js <input-json-file>
  node append-to-catalog.js --stdin < entries.json
  echo '{"id":"cd-999",...}' | node append-to-catalog.js --stdin

Examples:
  node append-to-catalog.js assets/enriched_batch_cd102-111.json
  cat new_entries.json | node append-to-catalog.js --stdin
`);
    process.exit(1);
  }
